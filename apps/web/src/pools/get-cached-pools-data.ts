...
const getV2PoolsAccData = async (blockNumber: number) => {
  if (USE_MONGO_CACHE) {
    const pools = await v2PoolsAccDataMongoCache.get(blockNumber.toString())
    if (pools) {
      return pools
    }
    // check lru cache
  ...
  if (USE_MONGO_CACHE) {
    await v2PoolsAccDataMongoCache.put(blockNumber.toString(), poolsMap)
  } else {
...
const getV3PoolsAccData = async (blockNumber: number) => {
  if (USE_MONGO_CACHE) {
    const pools = await v3PoolsAccDataMongoCache.get(blockNumber.toString())
    if (pools) {
      return pools
    }
    // check lru cache
  ...
  if (USE_MONGO_CACHE) {
    await v3PoolsAccDataMongoCache.put(blockNumber.toString(), poolsMap)
  } else {
...
const getProactivelyCachedV2Pools = localCachedProactive(
  async () => {
    if (USE_MONGO_CACHE) {
      console.log('proactive caching v2 pools with MongoDB')
    }
    ...
    if (USE_MONGO_CACHE) {
      await v2PoolsAccDataMongoCache.put(blockNumber.toString(), compressedV2Pools)
    } else {
...
const getProactivelyCachedV3Pools = localCachedProactive(
  async () => {
    if (USE_MONGO_CACHE) {
      console.log('proactive caching v3 pools with MongoDB')
    }
    const [blockNumber] = await getCachedBlockNumbers([Date.now()])
    ...
    if (USE_MONGO_CACHE) {
      await v3PoolsAccDataMongoCache.put(blockNumber.toString(), compressedV3Pools)
    } else {
