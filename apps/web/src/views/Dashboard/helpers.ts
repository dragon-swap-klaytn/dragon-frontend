...
export const mapMints = (mint: MintResponse) => {
  const transaction: Transaction = {
    ...
    sender: mint.to,
    token0Symbol: (mint.pair?.token0.symbol === 'WKLAY' ? 'WKAIA' : mint.pair?.token0.symbol) ?? '',
    token1Symbol: (mint.pair?.token1.symbol === 'WKLAY' ? 'WKAIA' : mint.pair?.token1.symbol) ?? '',
...
export const mapBurns = (burn: BurnResponse) => {
  const transaction: Transaction = {
    ...
    sender: burn.sender,
    token0Symbol: (burn.pair?.token0.symbol === 'WKLAY' ? 'WKAIA' : burn.pair?.token0.symbol) ?? '',
    token1Symbol: (burn.pair?.token1.symbol === 'WKLAY' ? 'WKAIA' : burn.pair?.token1.symbol) ?? '',
...
export const mapSwaps = (swap: SwapResponse) => {
  const transaction: Transaction = {
    ...
    sender: swap.from,
    token0Symbol: (swap.pair?.token0.symbol === 'WKLAY' ? 'WKAIA' : swap.pair?.token0.symbol) ?? '',
    token1Symbol: (swap.pair?.token1.symbol === 'WKLAY' ? 'WKAIA' : swap.pair?.token1.symbol) ?? '',
...
export const mapDayData = (tokenDayData: TokenDayData | PancakeDayData): V2TokenChartEntry => ({
...
export const mapPairDayData = (pairDayData: PairDayData): V2TokenChartEntry => ({
  date: pairDayData.date,
  volumeUSD: parseFloat(pairDayData.dailyVolumeUSD),
  liquidityUSD: parseFloat(pairDayData.reserveUSD),
})

type PoolOrTokenFetchFn = (skip: number, address: string) => Promise<{ data?: V2TokenChartEntry[]; error: boolean }>

type OverviewFetchFn = (skip: number) => Promise<{ data?: V2TokenChartEntry[]; error: boolean }>
...
export const fetchChartData = async (
  ...
  let chartEntries: V2TokenChartEntry[] = []
  let error = false
  let skip = 0
  let allFound = false
  ...
  while (!allFound && !error) {
    ...
    const { data, error: fetchError } = await getEntityDayDatas(skip)
    skip += 1000
    allFound = (data?.length ?? 0) < 1000 || skip > 2000
    error = fetchError
    if (data) {
  ...
  let timestamp = firstAvailableDayData?.date ?? SUBGRAPH_START_BLOCK[ChainId.KLAYTN]
...
export const fetchChartDataWithAddress = async (
  ...
  try {
    ...
    while (!allFound && !error) {
      ...
      skip += 100
      allFound = (data?.length ?? 0) < 100 || skip > 1000
      error = fetchError
      if (data) {
        chartEntries = chartEntries.concat(data)
      }
    ...
    console.error('Failed to fetch v2 token chart data', e)
    return {
      error: true,
      data: undefined,
    }
