...
import { Block, V2PoolData } from 'views/Dashboard/types'
...
export const fetchPoolData = async (
  ...
  try {
    ...
    const data = await request<PoolsQueryResponse>(subgraphUrls.v2Exchange, query)
...
export const fetchAllPoolDataWithAddress = async (blocks: Block[], poolAddresses: string[]) => {
  ...
  const { data } = await fetchPoolData(block24h.number, block48h.number, block7d.number, block14d.number, poolAddresses)
  ...
  const formatted = poolAddresses.reduce((accum: { [address: string]: { data: V2PoolData } }, address) => {
