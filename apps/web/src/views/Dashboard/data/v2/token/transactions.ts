...
export default async function fetchTokenTransactions(address: string) {
  try {
    const data = await request<TransactionResults>(subgraphUrls.v2Exchange, TOKEN_TRANSACTIONS(), {
      address,
    })
