...
import { Block, V3PoolData } from 'views/Dashboard/types'
 ...
 */
export async function fetchPoolDatas(poolAddresses: string[], blocks?: Block[]) {
  // get blocks from historic timestamps
  try {
    const [block24, block48, blockWeek] = blocks ?? []

    const data = await request<PoolDataResponse>(subgraphUrls.v3Exchange, POOLS_BULK(undefined, poolAddresses))

    const data24 = await request<PoolDataResponse>(subgraphUrls.v3Exchange, POOLS_BULK(block24?.number, poolAddresses))
    const data48 = await request<PoolDataResponse>(subgraphUrls.v3Exchange, POOLS_BULK(block48?.number, poolAddresses))
    const dataWeek = await request<PoolDataResponse>(
    ...
    const formatted = poolAddresses.reduce((accum: { [address: string]: V3PoolData }, address) => {
