...
import { Transaction, TransactionType } from 'views/Dashboard/types'
...
export default async function fetchV3Transactions() {
  try {
    const data = await request<TransactionResults>(subgraphUrls.v3Exchange, GLOBAL_TRANSACTIONS)
    ...
    const formatted = data.transactions.reduce((accum: Transaction[], t: TransactionEntry) => {
      const mintEntries = t.mints.map((m) => {
        return {
          ...
          sender: m.origin,
          token0Symbol: m.token0.symbol === 'WKLAY' ? 'WKAIA' : m.token0.symbol,
          token1Symbol: m.token1.symbol === 'WKLAY' ? 'WKAIA' : m.token1.symbol,
