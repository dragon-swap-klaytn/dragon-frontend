import { AutoRow, Flex, NotificationDot, QuestionHelper } from '@pancakeswap/uikit'
...
const AppHeader: React.FC<React.PropsWithChildren<Props>> = ({
  ...
  return (
    <div
      className={clsx('px-4 py-6 w-full flex flex-col items-start space-y-3', {
        'border-b border-gray-200': !borderHidden,
      })}
    >
      {backTo &&
        (typeof backTo === 'string' ? (
          <Link legacyBehavior passHref href={backTo}>
            <div className="flex items-center space-x-1 cursor-pointer hover:opacity-70">
              <ArrowLeft size={16} className="text-on-surface-primary shrink-0" />

              <span className="text-sm text-on-surface-primary">Back</span>
            </div>
          </Link>
        ) : (
          <button type="button" onClick={backTo} className="hover:opacity-70 flex items-center space-x-1">
            <ArrowLeft size={16} className="text-on-surface-primary" />
        ...
        <div className="flex items-center space-x-2 w-full">
          <div className="flex items-center space-x-2 w-full justify-between">
            <div className="flex flex-col items-start">
              ...
              >
                {typeof title === 'string' ? (
                  <h2 className="font-bold text-on-surface-primary text-lg">{title}</h2>
                ) : (
                  title
                )}
              ...
              {subtitle && <h4 className="text-sm text-on-surface-tertiary mt-0.5">{subtitle}</h4>}
            </div>
