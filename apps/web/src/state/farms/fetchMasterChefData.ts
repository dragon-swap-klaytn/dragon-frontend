...
const masterChefFarmCalls = (farm: SerializedFarm) => {
  ...
  const multiCallChainId = farmFetcher.isTestnet(quoteToken.chainId) ? ChainId.KLAYTN_TESTNET : ChainId.KLAYTN
...
export const fetchMasterChefData = async (
  ...
  const multiCallChainId = farmFetcher.isTestnet(chainId) ? ChainId.KLAYTN_TESTNET : ChainId.KLAYTN
