...
export async function getFarmsPricesFromSubgraph(
  chainId: number,
  farms: FarmData[],
  decimals: number,
): Promise<FarmWithPrices[]> {
  const query = gql`
    query fetchV2Pools($addresses: [String]) {
  ...
  `
  const { pairs } = await infoClientWithChain(chainId)?.request(query, {
    addresses: farms.map((f) => f.lpAddress.toLowerCase()),
  })

  const farmsWithPrices = farms.map((farm, i) => {
    const tokenPriceBusd = new BN(pairs[i].token0.derivedUSD)
    const quoteTokenPriceBusd = new BN(pairs[i].token1.derivedUSD)
    const lpTokenPrice = getFarmLpTokenPrice(farm, tokenPriceBusd, quoteTokenPriceBusd, decimals)
