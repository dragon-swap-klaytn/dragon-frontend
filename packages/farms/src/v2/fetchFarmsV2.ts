...
export async function farmV2FetchFarms({
  ...
  */
  ...
  const decimals = 18
  const farmsDataWithPrices = await getFarmsPricesFromSubgraph(chainId as number, farmsData, decimals)
...
export const fetchMasterChefData = async (
  ...
  try {
    ...
    const chainId = isTestnet ? ChainId.KLAYTN_TESTNET : ChainId.KLAYTN
